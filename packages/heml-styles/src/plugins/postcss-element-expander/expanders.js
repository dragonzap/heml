...
function replaceElementTagMentions(element, selector) {
  const processor = selectorParser((selectors) => {
    ...
    selectors.walkTags((node) => {
      if (node.value === element.tag) {
        nodesToReplace.push(node);
      }
    });

    nodesToReplace.forEach((node) =>
      node.replaceWith(selectorParser.string({ value: element.pseudos.root }))
    );
  });
    ...
    selectors.each((selector) => {
      ...
      selector.each((node) => {
        if (node.type === "tag" && node.value === element.tag) {
          ...
          const matchedPseudos = Object.entries(element.pseudos).filter(
            ([pseudo]) => {
...
function expandElementRule(element, selectors = [], originalRule) {
  ...
  for (const [ruleSelector, ruleDecls] of Object.entries(element.rules)) {
    ...
    if (!isRoot) {
      expandedRule.selectors = expandedRule.selectors.map(
        (selector) => `${selector} ${ruleSelector}`
      );
    }

    /** strip any non whitelisted props, run tranforms, gather used props */
    expandedRule.walkDecls((decl) => {
      const matchedRuleDecls = ruleDecls.filter(({ prop }) =>
        prop.test(decl.prop)
      );
      ...
      matchedRuleDecls.forEach(({ transform }) =>
