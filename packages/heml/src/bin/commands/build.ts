...
export function build(
  file: string,
  options: HEMLOptions & HEMLBuildOutput
): void {
  const filepath = path.resolve(file);
  const outputpath = path.resolve(
    options.output || file.replace(/\.heml$/, ".html")
  );
  ...
  try {
    ...
    renderHemlFile(filepath, options).then(({ html, metadata, errors }) => {
      ...
      log(
        errors.length
          ? `\n${errorBlock(" DONE ")} Compiled with errors to ${code(
              relativePath
            )} in ${metadata.time}ms\n`
          : `\n${successBlock(" DONE ")} Compiled successfully to ${code(
              relativePath
            )} in ${metadata.time}ms\n`
      );
      ...
      if (errors.length) {
        log(
          error(`${errors.length} ${errors.length > 1 ? "errors" : "error"} `)
        );
        errors.forEach((err) =>
