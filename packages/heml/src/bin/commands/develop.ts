...
export async function develop(
  file: string,
  options: HEMLOptions & HemlDevelopOptions
): Promise<void> {
  ...
  try {
    ...
    return startDevServer(path.dirname(filepath), port).then(
      ({ update, url }) => {
 ...
 */
function startDevServer(directory: string, port: number = 3000) {
  let url;
  const app = express();
  let preview = "";
