...
import { HEMLElement, HEMLElementContainsText, HEMLAttributes, HEMLNode } from './HemlElement';
import { HEMLOptions, HEMLCheerioStatic } from '@heml/parse';
import { Cheerio } from 'cheerio';

export { renderElement, HEMLElement, HEMLElementContainsText, HEMLAttributes, HEMLNode };
 ...
 */
export async function render($: HEMLCheerioStatic, options: HEMLOptions = {}): Promise<HEMLOutput> {
	...
	return renderElements(elements, globals).then(() => {
		...
		return { $, metadata: Meta ? (Meta.flush() as Record<string, any>) : {} } as HEMLOutput;
	});
}
...
async function renderElements(elements: Array<typeof HEMLElement>, globals: HEMLGlobals): Promise<void> {
	...
	const metaTagNames: string[] = filter(elements, { parent: ['head'] }).map((element) => element.name.toLowerCase());
	const nonMetaTagNames: string[] = difference(
		elements.map((element) => element.name.toLowerCase()),
		metaTagNames,
	);

	const $nodes = [...$.findNodes(metaTagNames) /** Render the meta elements first to last */, ...$.findNodes(nonMetaTagNames).reverse() /** Render the elements last to first/outside to inside */];

	return promiseQueue(elementMap, $nodes);
}

async function promiseQueue(elementMap: Record<string, typeof HEMLElement>, $nodes: Cheerio[], i: number = 0): Promise<void> {
	if (i >= $nodes.length) {
