...
export class HEMLElement<TAttributes extends HEMLAttributes = HEMLAttributes> {
	...
	public constructor(props: TAttributes, contents: HEMLNode) {
		this.props = { ...(this.constructor as typeof HEMLElement).defaultProps, contents, ...props };
	...
	protected render(): HEMLNode {
		if (this.props.contents !== 'string') {
			return Promise.all(flattenDeep(castArray(this.props.contents))).then((contents) => {
				return createHtmlElement(this.constructor.name.toLowerCase(), this.props, compact(flattenDeep(contents)).join('') || ' ');
			});
		}
	...
	private validAttrs($node: Cheerio): void {
		/** allow any attributes through */
		if (this.attrs === true) {
			return;
		}

		const allowedAttrs = ['id', 'class', 'dir', 'lang', 'accesskey', 'tabindex', 'title', 'translate', ...this.attrs];
		...
		if (foundNotAllowedAttrs.length > 0) {
			...
			throw new HEMLError(`Attribute${plural ? 's' : ''} ${foundNotAllowedAttrs.join(', ')} ${plural ? 'are' : 'is'} not allowed on ${this.constructor.name.toLowerCase()}.`, $node);
		}
	}

	private validChildren($node: Cheerio): void {
		if (Array.isArray(this.children)) {
			...
			if (foundRequiredChildren.length < this.children.length) {
				...
				throw new HEMLError(`${this.constructor.name.toLowerCase()} is missing required children: ${missingRequiredChildren}`, $node);
			}
		}
	}

	private validParent($node: Cheerio): void {
	...
	private validUnique($node: Cheerio, $: HEMLCheerioStatic): void {
...
export class HEMLElementContainsText<TAttributes extends HEMLAttributes = HEMLAttributes> extends HEMLElement<TAttributes> {
	public constructor(props: TAttributes, contents: HEMLNode) {
